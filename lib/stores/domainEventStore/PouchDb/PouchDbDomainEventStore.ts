...
class PouchDbDomainEventStore implements DomainEventStore {
  protected db: any;

  protected dbNames: DbNames;
  ...
  protected constructor ({ db, collectionNames, collections }: {
    db: any,
    collectionNames: DbNames;
    collections: {
      domainEvents: Collection<any>;
      snapshots: Collection<any>;
    };
  ...
  public static async create ({ hostName, port, userName, password, database, collectionNames }: {
    ...
    database: string;
    collectionNames: DbNames;
  }): Promise<PouchDbDomainEventStore> {
    ...
    db.on('close', PouchDbDomainEventStore.onUnexpectedClose);
    ...
    const domainEventStore = new PouchDbDomainEventStore({
      client,
      db,
      collectionNames,
      collections
  ...
  public async destroy (): Promise<void> {
    this.db.removeListener('close', PouchDbDomainEventStore.onUnexpectedClose);
